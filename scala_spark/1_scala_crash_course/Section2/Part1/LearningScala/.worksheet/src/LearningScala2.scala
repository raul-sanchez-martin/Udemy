object LearningScala2 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(141); 
  ...
  do { println(x); x+=1 } while (x <= 10);$skip(154); val res$0 = 
   ...
   {val x = 10; x + 20};System.out.println("""res0: Int = """ + $show(res$0));$skip(82); 
	 ...
	 println({val x = 10; x + 20});$skip(279); 
	 ...
	 var last_num : Int =  1;System.out.println("""last_num  : Int = """ + $show(last_num ));$skip(30); ;
	 var last_num_pre : Int = 0;System.out.println("""last_num_pre  : Int = """ + $show(last_num_pre ));$skip(279); ;
	 ...
	 for(idx <- 0 to 9){
	 		...
	 		if(idx != 0 & idx != 1){
			 		var new_num : Int = last_num_pre + last_num;
			 		println(new_num);
