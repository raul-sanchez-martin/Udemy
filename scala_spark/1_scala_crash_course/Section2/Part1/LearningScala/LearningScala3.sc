object LearningScala3 {
  ...
  // Then, do the same thing using a function literal instead of a separate, named function.
	...
	println(myString.toUpperCase());          //> FOO
	...
	def myUpperCase(x:String):String = {
		x.toUpperCase()
	}                                         //> myUpperCase: (x: String)String
	
	
	def transformUpper(x:String, f: String => String): String = {
		f(x)
	}                                         //> transformUpper: (x: String, f: String => String)String

	transformUpper("foo", myUpperCase)        //> res3: String = FOO
	
	transformUpper("foo", x => x.toUpperCase())
                                                  //> res4: String = FOO
	
  myUpperCase("foo")                              //> res5: String = FOO
}